    ...
    public class CreateExcel
    {
        ...
        private Stream RenderDataTableToExcel(DataTable SourceTable)
        {
            ...
            foreach (DataRow row in SourceTable.Rows)
            {
                ...
                foreach (DataColumn column in SourceTable.Columns)
                {
                    if (column.ColumnName == SajetClass.SajetCommon.SetLanguage("TARGET_QTY")
                        || column.ColumnName == SajetClass.SajetCommon.SetLanguage("INPUT_QTY")
                        ||column.ColumnName == SajetClass.SajetCommon.SetLanguage("OUTPUT_QTY")
                        || column.ColumnName == SajetClass.SajetCommon.SetLanguage("SCRAP_QTY"))
                    {
                        if (double.TryParse(row[column].ToString(),out double d))
        ...
        public void RenderDataTableToExcel(DataGridView dgvSource, string FileName)
        {
            DataSet dsGrid = new DataSet();
            dsGrid.Tables.Add();
            ...
            for (int i = 0; i <= dgvSource.Columns.Count - 1; i++)
            {
                if (dgvSource.Columns[i].Visible)
                {
                    lsOrder.Add(dgvSource.Columns[i].DisplayIndex);
                    htSource.Add(dgvSource.Columns[i].DisplayIndex, dgvSource.Columns[i].HeaderText);
                    dsGrid.Tables[0].Columns.Add(dgvSource.Columns[i].HeaderText);
                }
            }
            lsOrder.Sort();
            ...
            for (int i = 0; i <= dgvSource.Rows.Count - 1; i++)
            {
                dsGrid.Tables[0].Rows.Add();
                int iCol = -1;
                for (int j = 0; j <= dgvSource.Columns.Count - 1; j++)
                {
                    if (dgvSource.Columns[j].Visible)
                    {
                        iCol = iCol + 1;
                        dsGrid.Tables[0].Rows[i][iCol] = dgvSource.Rows[i].Cells[j].Value;
                    }
                }
            }
            ...
            //更改欄位順序
            int k = 0;
            foreach (int iNum in lsOrder)
            {
                dsGrid.Tables[0].Columns[htSource[iNum].ToString()].SetOrdinal(k);
                k++;
            }

            MemoryStream ms = RenderDataTableToExcel(dsGrid.Tables[0]) as MemoryStream;
